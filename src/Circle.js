import React from 'react';

import Card from './Card.js';


class Circle extends React.Component {
    constructor(props) {
        super(props)

        this.state = {
            radius: 130,
            cards: [],
            theta: 0.0,
            numberOfCard: 4,
        }

        this.temp_theta = 0.0;
        this.anim_id = null;
    }

    let 

    componentDidMount() {
        let center_of_wheel = {
            x: parseFloat(this.wheel.style.width) / 2,
            y: parseFloat(this.wheel.style.height) / 2
        }
        let temp_cards = [];
        
        for (let i = 0; i < this.state.numberOfCard; i++) {
            temp_cards.push(
                <Card radius={this.state.radius} radian_interval={(Math.PI / (this.state.numberOfCard/2)) * i} center={center_of_wheel} key={`card_${i}`} />
            );
        }

        this.setState({ cards: temp_cards });
    }

    handle_scroll = event => {
        clearTimeout(this.anim_id);
        this.wheel.style.transform = `translate(-50%, -50%) rotate(${this.temp_theta + (event.deltaY * 0.5)}deg)`;
        this.temp_theta += (event.deltaY * 0.5);

        this.anim_id = setTimeout(() => {
            this.setState({ theta: this.temp_theta });
        }, 150);
    }

    render() {
        return (
            <div onWheel={this.handle_scroll} ref={ref_id => this.wheel = ref_id} style={styles.wheel}>
                {this.state.cards}
            </div>
        )

    }
}

const styles = {
    wheel: {
        margin: '0',
        padding: '0',
        position: 'absolute',
        top: '50%',
        left: '50%',
        transform: 'translate(-50%, -50%)',
        height: '250px',
        width: '180px',
        backgroundColor: 'red',
        zIndex: 10,
    }
}

export default Circle;